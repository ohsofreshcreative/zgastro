{{--
  Cart Page
  This template can be overridden by copying it to yourtheme/resources/views/woocommerce/cart/cart.blade.php.
--}}
@php
        ...
        @if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key))
          ...
          <tr class="woocommerce-cart-form__cart-item {{ esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)) }}">
            ...
            <td class="product-remove">
              {!! apply_filters('woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s">&times;</a>', esc_url(wc_get_cart_remove_url($cart_item_key)), esc_html__('Remove this item', 'woocommerce'), esc_attr($product_id), esc_attr($_product->get_sku())), $cart_item_key) !!}
            </td>
            ...
            <td class="product-name" data-title="{{ esc_attr_e('Product', 'woocommerce') }}">
              ...
              @if ($_product->backorders_require_notification() && $_product->is_on_backorder($cart_item['quantity']))
                <p class="backorder_notification">{{ esc_html_e('Available on backorder', 'woocommerce') }}</p>
              @endif
            </td>
            ...
            <td class="product-quantity" data-title="{{ esc_attr_e('Quantity', 'woocommerce') }}">
              @if ($_product->is_sold_individually())
                @php $product_quantity = sprintf('1 <input type="hidden" name="cart[%s][qty]" value="1" />', $cart_item_key) @endphp
              @else
                @php $product_quantity = woocommerce_quantity_input(['input_name' => "cart[{$cart_item_key}][qty]", 'input_value' => $cart_item['quantity'], 'max_value' => $_product->get_max_purchase_quantity(), 'min_value' => '0', 'product_name' => $_product->get_name()], $_product, false) @endphp
              @endif
              {!! apply_filters('woocommerce_cart_item_quantity', $product_quantity, $cart_item_key, $cart_item) !!}
            </td>

            <td class="product-subtotal" data-title="{{ esc_attr_e('Subtotal', 'woocommerce') }}">
      ...
      <tr>
        <td colspan="6" class="actions">
          @if (wc_coupons_enabled())
            <div class="coupon">
              <label for="coupon_code">{{ esc_html_e('Coupon:', 'woocommerce') }}</label>
              <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="{{ esc_attr_e('Coupon code', 'woocommerce') }}" />
          ...
          @php do_action('woocommerce_cart_actions'); wp_nonce_field('woocommerce-cart', 'woocommerce-cart-nonce') @endphp
        </td>
      </tr>
