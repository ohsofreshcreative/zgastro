...
<li {!! wc_product_class('', $product) !!}>
	<figure class="woocommerce-product-figure">
		<a href="{{ get_permalink() }}">
			<img src="{{ get_the_post_thumbnail_url($product->get_id(), 'large') }}" alt="{{ get_the_title() }}" class="img-m" />
		</a>
	</figure>
	...
	@php
		...
		$parent_cat = collect($categories)
			->filter(function ($cat) {
				return $cat->parent === 0 && $cat->slug !== 'wszystkie-produkty'; // 
			})
			->first();
	@endphp
