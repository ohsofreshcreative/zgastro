<div data-gsap-anim="section" class="block-category-posts -smt {{ $flip ? 'flip' : '' }} layout-{{ $layout }}">
	<div class="c-main">
		<div class="w-full lg:w-1/2 m-auto">
			<h2 data-gsap-element="header" class="text-center">{{ $posts_settings['title'] }}</h2>
			<p data-gsap-element="text" class="text-center text-lg">{{ $posts_settings['txt'] }}</pack>
				<!-- @if($category_id)
		...
		<div class="posts-container grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
			...
			<div data-gsap-element="card" class="post-item bg-white b-radius">
